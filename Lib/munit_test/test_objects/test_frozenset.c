...
static MunitResult test_call_type(const MunitParameter params[], fixture_t *fixture)
{
    ...
    // Invalid arguments.
    {
        ypObject *set_empty = yp_setN(0);
        ypObject *args_two = yp_tupleN(N(set_empty, set_empty));
        ypObject *kwargs_iterable = yp_frozendictK(K(str_iterable, set_empty));
        ypObject *kwargs_cls = yp_frozendictK(K(str_cls, type->yp_type));
        ypObject *kwargs_rand = yp_frozendictK(K(str_rand, set_empty));

        assert_raises(yp_callN(type->yp_type, N(set_empty, set_empty)), yp_TypeError);
