...
import python_test.support
from python_test.support import import_helper
from python_test.support import warnings_helper
import python_test.string_tests
import python_test.list_tests
from python_test.support import bigaddrspacetest, MAX_Py_ssize_t
...
class BaseBytesTest:
    ...
    def test_from_iterable(self):
        b = self.type2test(yp_range(256))
        self.assertEqual(yp_len(b), 256)
        self.assertEqual(yp_list(b), yp_list(yp_range(256)))

        # Non-sequence iterable.
        b = self.type2test({42})
        self.assertEqual(b, b"*")
        b = self.type2test({43, 45})
        self.assertIn(yp_tuple(b), yp_set({(43, 45), (45, 43)}))

        # Iterator that has a __length_hint__.
        b = self.type2test(iter(yp_range(256)))
        self.assertEqual(yp_len(b), 256)
        self.assertEqual(yp_list(b), yp_list(yp_range(256)))

        # Iterator that doesn't have a __length_hint__.
        b = self.type2test(i for i in yp_range(256) if i % 2)
        self.assertEqual(yp_len(b), 128)
        self.assertEqual(yp_list(b), yp_list(yp_range(256))[1::2])

    @yp_unittest.skip_user_defined_types
    def test_from_iterable_no_iter(self):
    ...
    def test_from_tuple(self):
        ...
        b = self.type2test(yp_tuple(yp_range(256)))
        self.assertEqual(yp_len(b), 256)
        self.assertEqual(yp_list(b), yp_list(yp_range(256)))
    ...
    def test_from_list(self):
        ...
        b = self.type2test(yp_list(yp_range(256)))
        self.assertEqual(yp_len(b), 256)
        self.assertEqual(yp_list(b), yp_list(yp_range(256)))
        b = self.type2test([1, 2, 3])
        self.assertEqual(b, b"\x01\x02\x03")
    ...
    def test_from_mutating_list(self):
        ...
        class Y:
            def __index__(self):
                ...
                return 42
        a = [Y()]
        self.assertEqual(yp_bytes(a), b'*' * 1000)  # should not crash
    ...
    @yp_unittest.skip_user_defined_types
    def test_from_buffer_index_fallback(self):
    ...
    @python_test.support.requires_resource('cpu')
    ...
    def test_concat(self):
        ...
        self.assertEqual(b1 + yp_bytes(b"def"), b"abcdef")
        self.assertEqual(yp_bytes(b"def") + b1, b"defabc")
        self.assertRaises(TypeError, lambda: b1 + "def")
        self.assertRaises(TypeError, lambda: "abc" + b2)

    def test_repeat(self):
        for b in yp_bytes(b"abc"), self.type2test(b"abc"):
    ...
    def test_contains(self):
        ...
        for f in yp_bytes, yp_bytearray:
            self.assertIn(f(b""), b)
    ...
    def test_center(self):
        ...
        b = self.type2test(b'abc')
        for fill_type in (yp_bytes, yp_bytearray):
    ...
    def test_free_after_iterating(self):
        python_test.support.check_free_after_iterating(self, iter, self.type2test)
        python_test.support.check_free_after_iterating(self, reversed, self.type2test)
    ...
    def test_translate(self):
        ...
        self.assertRaises(ValueError, b.translate, yp_bytes(range(255)))
...
class BytesTest(BaseBytesTest, yp_unittest.TestCase):
    ...
    def test_custom(self):
        class A:
            def __bytes__(self):
                return yp_bytes(b'abc')
        self.assertEqual(yp_bytes(A()), b'abc')
        class A: pass
        self.assertRaises(TypeError, yp_bytes, A())
        class A:
            def __bytes__(self):
                return None
        self.assertRaises(TypeError, yp_bytes, A())
        ...
        class A(str):
            def __bytes__(self):
                return b'abc'
        self.assertEqual(yp_bytes(A('\u20ac')), b'abc')
        self.assertEqual(yp_bytes(A('\u20ac'), 'iso8859-15'), b'\xa4')
        # Issue #24731
        class A:
            def __bytes__(self):
                return OtherBytesSubclass(b'abc')
        self.assertEqual(yp_bytes(A()), b'abc')
        self.assertIs(type(yp_bytes(A())), OtherBytesSubclass)
    ...
    def test_bytes_blocking(self):
        class IterationBlocked(list):
            __bytes__ = None
        i = [0, 1, 2, 3]
        self.assertEqual(yp_bytes(i), b'\x00\x01\x02\x03')
        self.assertRaises(TypeError, yp_bytes, IterationBlocked(i))
        ...
        class IntBlocked(int):
            __bytes__ = None
        self.assertEqual(yp_bytes(3), b'\0\0\0')
        self.assertRaises(TypeError, yp_bytes, IntBlocked(3))
        ...
        class BytesSubclassBlocked(yp_bytes):
            __bytes__ = None
        self.assertEqual(yp_bytes(b'ab'), b'ab')
        self.assertRaises(TypeError, yp_bytes, BytesSubclassBlocked(b'ab'))

        class BufferBlocked(yp_bytearray):
            __bytes__ = None
        ba, bb = yp_bytearray(b'ab'), BufferBlocked(b'ab')
        self.assertEqual(yp_bytes(ba), b'ab')
        self.assertRaises(TypeError, yp_bytes, bb)

    def test_repeat_id_preserving(self):
        a = yp_bytes(b'123abc1@')
        b = yp_bytes(b'456zyx-+')
        self.assertIs(a, a)
        self.assertIsNot(a, b)
        self.assertIsNot(a, a * -4)
        self.assertIsNot(a, a * 0)
        self.assertIs(a, a * 1)
        self.assertIs(a, 1 * a)
        self.assertIsNot(a, a * 2)

    @yp_unittest.skip_user_defined_types
    def test_repeat_id_preserving(self):
        class SubBytes(yp_bytes):
            pass

        s = SubBytes(b'qwerty()')
        self.assertIs(s, s)
        self.assertIsNot(s, s * -4)
        self.assertIsNot(s, s * 0)
        self.assertIsNot(s, s * 1)
        self.assertIsNot(s, 1 * s)
        self.assertIsNot(s, s * 2)
...
class ByteArrayTest(BaseBytesTest, yp_unittest.TestCase):
    ...
    def test_setitem(self):
        b = yp_bytearray([1, 2, 3])
        b[1] = 100
        self.assertEqual(b, yp_bytearray([1, 100, 3]))
        b[-1] = 200
        self.assertEqual(b, yp_bytearray([1, 100, 200]))
        b[0] = Indexable(10)
        self.assertEqual(b, yp_bytearray([10, 100, 200]))
        try:
            b[3] = 0
    ...
    def test_fifo_overrun(self):
        ...
        b += yp_bytes(2)  # Append exactly the number of deleted bytes
    ...
    def test_alloc(self):
        ...
        for i in yp_range(100):
            b += yp_bytes(b"x")
            alloc = b.__alloc__()
            self.assertGreater(alloc, yp_len(b))  # including trailing null byte
            if alloc not in seq:
                seq.append(alloc)
    ...
    def test_init_alloc(self):
        b = yp_bytearray()
        def g():
            for i in yp_range(1, 100):
                yield i
                a = yp_list(b)
                self.assertEqual(a, yp_list(yp_range(1, yp_len(a)+1)))
                self.assertEqual(yp_len(b), yp_len(a))
                self.assertLessEqual(yp_len(b), i)
                alloc = b.__alloc__()
                self.assertGreater(alloc, yp_len(b))  # including trailing null byte
        b.__init__(g())
        self.assertEqual(yp_list(b), yp_list(yp_range(1, 100)))
        self.assertEqual(yp_len(b), 99)
        alloc = b.__alloc__()
        self.assertGreater(alloc, yp_len(b))

    def test_extend(self):
        orig = yp_bytes(b'hello')
        a = yp_bytearray(orig)
        a.extend(a)
    ...
    def test_remove(self):
        ...
        c.remove(127)
        self.assertEqual(c, yp_bytes([126, 128, 129]))
        c.remove(129)
        self.assertEqual(c, yp_bytes([126, 128]))

    def test_pop(self):
        b = yp_bytearray(b'world')
        self.assertEqual(b.pop(), ord('d'))
        self.assertEqual(b.pop(0), ord('w'))
        self.assertEqual(b.pop(-2), ord('r'))
        self.assertRaises(IndexError, lambda: b.pop(10))
        self.assertRaises(IndexError, lambda: yp_bytearray().pop())
        # test for issue #6846
        self.assertEqual(yp_bytearray(b'\xff').pop(), 0xff)

    def test_nosort(self):
        self.assertRaises(AttributeError, lambda: yp_bytearray().sort())
    ...
    def test_partition_bytearray_doesnt_share_nullstring(self):
        ...
        self.assertEqual(c, b"")
        # Same for rpartition
        b, c, a = yp_bytearray(b"x").rpartition(b"y")
        self.assertEqual(b, b"")
        self.assertEqual(c, b"")
        self.assertIsNot(b, c)
        b += b"!"
        self.assertEqual(c, b"")
        c, b, a = yp_bytearray(b"x").rpartition(b"y")
        self.assertEqual(b, b"")
        self.assertEqual(c, b"")
    ...
    @yp_unittest.skip_not_applicable
    @python_test.support.cpython_only
    def test_obsolete_write_lock(self):
        from _testcapi import getbuffer_with_null_view
        self.assertRaises(BufferError, getbuffer_with_null_view, yp_bytearray())
    ...
    test_exhausted_iterator = python_test.list_tests.CommonTest.test_exhausted_iterator
...
class AssortedBytesTest(yp_unittest.TestCase):
    #
    ...
    @yp_unittest.skip_not_applicable
    @python_test.support.requires_docstrings
    def test_doc(self):
        self.assertIsNotNone(yp_bytearray.__doc__)
        self.assertTrue(yp_bytearray.__doc__.startswith("bytearray("), yp_bytearray.__doc__)
        self.assertIsNotNone(yp_bytes.__doc__)
        self.assertTrue(yp_bytes.__doc__.startswith("bytes("), yp_bytes.__doc__)
...
class FixedStringTest(python_test.string_tests.BaseTest):
...
class SubclassTest:
    ...
    def test_basic(self):
        ...
        a, b = yp_bytes(b"abcd"), yp_bytes(b"efgh")
    ...
    def test_fromhex(self):
        ...
        class B2(self.basetype):
            def __init__(me, *args, **kwargs):
                if self.basetype is not yp_bytes:
...
class ByteArraySubclass(yp_bytearray):
    pass
...
class ByteArraySubclassTest(SubclassTest, yp_unittest.TestCase):
    ...
    def test_init_override(self):
        class subclass(yp_bytearray):
            def __init__(me, newarg=1, *args, **kwargs):
                yp_bytearray.__init__(me, *args, **kwargs)
        x = subclass(4, yp_bytes(b"abcd"))
        x = subclass(4, source=yp_bytes(b"abcd"))
        self.assertEqual(x, yp_bytes(b"abcd"))
        x = subclass(newarg=4, source=yp_bytes(b"abcd"))
        self.assertEqual(x, yp_bytes(b"abcd"))
