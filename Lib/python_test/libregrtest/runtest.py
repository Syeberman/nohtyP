...
from python_test.libregrtest.cmdline import Namespace
from python_test.libregrtest.save_env import saved_test_environment
from python_test.libregrtest.utils import clear_caches, format_duration, print_warning
...
STDTESTS = [
    # XXX Only run the tests found in python_test.
    # 'test_grammar',
    # 'test_opcodes',
    # 'test_dict',
    # 'test_builtin',
...
def get_abs_module(ns: Namespace, test_name: str) -> str:
    if test_name.startswith('python_test.') or ns.testdir:
        return test_name
    else:
        # Import it from the python_test package
        return 'python_test.' + test_name
...
def _runtest_inner2(ns: Namespace, test_name: str) -> bool:
    ...
    if ns.huntrleaks:
        from python_test.libregrtest.refleak import dash_R
